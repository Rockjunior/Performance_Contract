...
data <- data.frame(
  ...
  Efficiency_Score = sample(80:100, 50, replace = TRUE),
  Compliance_Score = sample(85:100, 50, replace = TRUE),
  Teamwork_Score = sample(70:100, 50, replace = TRUE),
  Community_Feedback_Score = sample(75:95, 50, replace = TRUE)
)

# Define weights
weights <- c(Revenue = 0.20, Clients = 0.15, Satisfaction = 0.15, Efficiency = 0.20, Compliance = 0.15, Teamwork = 0.10, Community = 0.05)
...
data <- data %>%
  mutate(
    ...
    Norm_Efficiency = (Efficiency_Score - min(Efficiency_Score)) / (max(Efficiency_Score) - min(Efficiency_Score)),
    Norm_Compliance = (Compliance_Score - min(Compliance_Score)) / (max(Compliance_Score) - min(Compliance_Score)),
    Norm_Teamwork = (Teamwork_Score - min(Teamwork_Score)) / (max(Teamwork_Score) - min(Teamwork_Score)),
    Norm_Community = (Community_Feedback_Score - min(Community_Feedback_Score)) / (max(Community_Feedback_Score) - min(Community_Feedback_Score))
  )
...
data <- data %>%
  mutate(
    Total_Score = (Norm_Revenue * weights["Revenue"]) +
                  ...
                  (Norm_Efficiency * weights["Efficiency"]) +
                  (Norm_Compliance * weights["Compliance"]) +
                  (Norm_Teamwork * weights["Teamwork"]) +
                  (Norm_Community * weights["Community"])
  )
